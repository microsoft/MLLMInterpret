...
class Trace(contextlib.AbstractContextManager):
    ...
    def __init__(
        ...
        # Retainer
        retainer = self
        if edit_output == 'single':
            #print(f'In edit output .... ')
            self.token_index = layer[0]
            self.layer = layer[1]
        ...
        if self.layer is not None:
            module = get_module(module, self.layer)

        
        # Retain hook function
        def retain_hook(m, inputs, output):
            # False - as input
            if retain_input:
                print(f'Retainer Input ... ###')
            ...
            if edit_output:
                ...
                if edit_output == 'single':
                    ...
                    if 'self_attn' in self.layer:
                        output[0][2][self.token_index] = output[0][0][self.token_index]
                    
                    elif 'mlp' in self.layer:
                        output[2][self.token_index] = output[0][self.token_index]
